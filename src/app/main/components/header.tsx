...
export function AppHeader() {
  ...
  const allMainNavLinks = [
    ...
    { href: "/main/subjects", label: "Subjects", icon: <Book className="h-5 w-5" /> },
    { href: "/main/timetable", label: "My Schedule", icon: <Calendar className="h-5 w-5" /> },
    { href: "/main/focus", label: "Focus Timer", icon: <Timer className="h-5 w-5" /> },
  ];
  ...
  const aiToolsLinks = [
    ...
    { href: "/main/ai-mnemonics", label: "Mnemonics", icon: <Lightbulb className="h-5 w-5" /> },
  ...
  return (
    <header className="flex h-24 items-center gap-6 border-b bg-background/60 backdrop-blur-3xl px-8 lg:px-12 sticky top-0 z-30 shadow-2xl transition-all">
      <div className="flex items-center gap-6">
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button variant="ghost" size="icon" className="shrink-0 lg:hidden rounded-2xl border-2 hover:bg-primary/10 transition-all">
              <Menu className="h-7 w-7 text-primary" />
              <span className="sr-only">Toggle drawer</span>
            </Button>
          </SheetTrigger>
          <SheetContent side="left" className="flex flex-col rounded-r-[4rem] border-r-8 border-primary/10 w-[340px] p-10 bg-background/95 backdrop-blur-3xl">
            <ScrollArea className="h-full pr-4">
              <nav className="grid gap-6 text-lg font-medium pb-10">
                <Link
                  href="/main/dashboard"
                ...
                {mainNavLinks.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="flex items-center gap-5 rounded-3xl px-6 py-3 text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all font-bold text-[14px]"
                  >
                ...
                <div className="my-4 border-t-2 border-primary/5">
                  <p className="text-[10px] font-bold text-primary uppercase px-6 pt-4 tracking-widest">Resources</p>
                </div>
                ...
                <div className="my-4 border-t-2 border-primary/5">
                  <p className="text-[10px] font-bold text-primary uppercase px-6 pt-4 tracking-widest">AI Tools</p>
                </div>
      ...
      <div className="flex-1 flex items-center justify-center max-w-2xl mx-auto hidden md:flex">
        <form onSubmit={handleSearch} className="relative w-full group">
          ...
          <Input 
            ...
            placeholder="Search subjects, tasks, or tools..." 
      ...
      <div className="flex items-center gap-6">
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="relative rounded-2xl hover:bg-primary/10 transition-all h-12 w-12 border-2 border-transparent">
                    <Bell className="h-6 w-6" />
                    {announcements.length > 0 && (
                      <span className="absolute top-3 right-3 h-3 w-3 bg-primary rounded-full border-4 border-background animate-pulse shadow-[0_0_15px_hsl(var(--primary))]" />
                    )}
                </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-[400px] rounded-[3rem] border-4 border-primary/10 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.4)] p-0 mt-4 bg-background/95 backdrop-blur-3xl overflow-hidden">
                <DropdownMenuLabel className="font-black uppercase text-[11px] tracking-[0.4em] px-10 py-8 text-primary/60 border-b-2 border-primary/5 bg-primary/5">Notifications</DropdownMenuLabel>
                <ScrollArea className="max-h-[500px]">
                  <div className="p-4 space-y-2">
                    <DropdownMenuItem className="rounded-[2rem] p-6 cursor-default hover:bg-primary/5 transition-all border-4 border-transparent mb-2 outline-none">
                        <div className="flex gap-5 items-start">
                            <div className="h-12 w-12 rounded-2xl bg-primary/10 flex items-center justify-center shrink-0 border-2 border-primary/20">
                              <Sparkles className="h-6 w-6 text-primary animate-float fill-primary/20" />
                            </div>
                            <div className="flex flex-col gap-1">
                                <p className="font-bold text-base text-primary uppercase tracking-tight">Welcome to StudySpark</p>
                                <p className="text-[13px] text-muted-foreground font-medium leading-relaxed">Hello! We're glad you're here. Let's make learning exciting and simple together.</p>
                            </div>
                        </div>
                    </DropdownMenuItem>
                    ...
                    {loading ? (
                      ...
                      announcements.slice(0, 5).map((ann) => (
                        <DropdownMenuItem key={ann.id} asChild className="outline-none">
                          <Link href="/main/announcements" className="rounded-[2rem] p-6 cursor-pointer hover:bg-primary/5 transition-all border-4 border-transparent hover:border-primary/10 mb-2 block">
                            <div className="flex gap-5 items-start">
                                <div className="h-12 w-12 rounded-2xl bg-emerald-500/10 flex items-center justify-center shrink-0 border-2 border-emerald-500/20">
                                  <Megaphone className="h-6 w-6 text-emerald-500 animate-pulse" />
                                </div>
                                <div className="flex flex-col gap-1">
                                    <p className="font-bold text-base text-emerald-600 uppercase tracking-tight truncate max-w-[240px]">{ann.title}</p>
                                    <p className="text-[13px] text-muted-foreground font-medium leading-relaxed line-clamp-2">{ann.content}</p>
                                    <p className="text-[10px] font-bold uppercase text-muted-foreground/40 mt-1">{ann.teacherName}</p>
                                </div>
                            </div>
                          </Link>
                        </DropdownMenuItem>
